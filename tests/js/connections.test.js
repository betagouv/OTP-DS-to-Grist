  ...
  it(
    ...
    async () => {
      ...
      const mockConfigResponse = {
        ok: true,
        json: jest.fn().mockResolvedValue({ configs: [ { ds_api_token: 'config-token' } ] })
      }
      global.fetch
...
describe('testGristConnection', () => {
  ...
  it(
    'should reload config when no key in input but config exists', async () => {
      ...
      const mockConfigResponse = {
        ok: true,
        json: jest.fn().mockResolvedValue({ configs: [{grist_api_key: 'config-key'}] })
      }
      global.fetch
