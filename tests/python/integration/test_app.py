...
class TestErrorHandling:
    ...
    def test_api_start_sync_task_failure_before_refactor(
        self, mock_load, mock_run, client
    ):
    ...
    def test_scheduled_sync_job_success(self, mock_session):
        ...
        with patch("app.config_manager.load_config_by_id") as mock_load_config:
            ...
            with patch.object(
                sync_task_manager, "run_synchronization_task"
            ) as mock_sync:
    ...
    @patch("app.SessionLocal")
    def test_scheduled_sync_job_api_error_disables_schedule(self, mock_session):
        """Test que error_code=2 désactive le planning et crée SyncLog"""
        from app import scheduled_sync_job
        from app import EXIT_CODE_EXTERNAL_API_ERROR
        ...
        def query_side_effect(*args, **kwargs):
            if args[0].__name__ == "OtpConfiguration":
                return MagicMock(
                    filter_by=MagicMock(
                        return_value=MagicMock(
                            first=MagicMock(return_value=mock_config)
                        )
                    )
                )
            elif args[0].__name__ == "UserSchedule":
                return MagicMock(
                    filter_by=MagicMock(
                        return_value=MagicMock(
                            first=MagicMock(return_value=mock_schedule)
                        )
                    )
                )
            return MagicMock()
        ...
        with patch("app.config_manager.load_config_by_id") as mock_load_config:
            ...
            with patch.object(
                ...
                mock_sync.return_value = {
                    "success": False,
                    "message": "API error: Token expiré",
    ...
    @patch("app.SessionLocal")
    def test_scheduled_sync_job_general_error_disables_schedule(self, mock_session):
        """Test que error_code=1 (érreur générale) désactive le planning"""
        from app import scheduled_sync_job
