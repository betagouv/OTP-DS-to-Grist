    ...
    def run_synchronization_task(self, config, progress_callback=None, log_callback=None):
        try:
            ...
            if log_callback:
                ...
                log_callback(f"✓ Filtre date début: {date_debut}" if date_debut else "○ Date début: AUCUN FILTRE (tous les dossiers)")
                log_callback(f"✓ Filtre date fin: {date_fin}" if date_fin else "○ Date fin: AUCUN FILTRE (tous les dossiers)")
                log_callback(f"✓ Filtre statuts: {statuts}" if statuts else "○ Statuts: AUCUN FILTRE (tous les dossiers)")
                log_callback(f"✓ Filtres groupes: {groupes}" if groupes else "○ Groupes: AUCUN FILTRE (tous les dossiers)")
                log_callback("====================================")
